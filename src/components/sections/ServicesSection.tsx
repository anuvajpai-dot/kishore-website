...
import { Camera, Baby, Heart, Users, Stars, Cake } from "lucide-react";
...
const services = [
  ...
  },
  {
    ...
    description: "From vows to dancing — your love story documented with emotion and artistry.",
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="font-serif text-4xl md:text-5xl text-[#f5f0eb] font-light">
            Every{" "}
            <span className="gold-shimmer font-semibold">Occasion</span>{" "}
            Deserves
            <br />
            a Masterpiece
          </h2>
          <div className="w-16 h-px bg-gold-500 mx-auto mt-6 mb-4" />
          <p className="text-[#707070] text-sm tracking-widest uppercase">
          ...
          {services.map((service, i) => (
            <motion.div
              ...
              className={`relative p-8 border transition-all duration-300 group hover:-translate-y-1 ${
                service.featured
                  ? "border-gold-500/50 bg-gradient-to-b from-gold-500/5 to-transparent"
                  : "border-gold-500/10 bg-[#111] hover:border-gold-500/30"
              }`}
            >
              {service.featured && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="bg-gold-500 text-[#0a0a0a] text-[10px] tracking-widest uppercase font-bold px-4 py-1">
                    Most Popular
                  </span>
                </div>
              )}
              ...
              <h3 className="font-serif text-xl text-[#f5f0eb] font-medium mb-3">{service.title}</h3>
              <p className="text-[#707070] text-sm leading-relaxed mb-6">{service.description}</p>
              <ul className="space-y-2 mb-8">
                {service.features.map((f) => (
                  <li key={f} className="flex items-center gap-2 text-[#808080] text-xs">
              ...
              <Link
                ...
                className={`text-xs tracking-widest uppercase transition-colors ${
                  service.featured
                    ...
                    : "text-[#606060] hover:text-gold-500"
                }`}
              >
                Enquire Now →
              </Link>
